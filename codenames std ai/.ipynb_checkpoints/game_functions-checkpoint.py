...
def bot(player,board,model,min_sim=0.40):
    ...
    hints_df2=hints_df[hints_df.effective==1].sort_values(['valid_number','sum_sim'],ascending=False)
    if len(hints_df2)>0:
        hint=tuple(hints_df2.iloc[0][['valid_number','hint','target_words']])
        print(f'{player} hint {hint[1]} targeted: {hint[2]}')
        return f'{str(hint[0])}: {hint[1]}'
    else: 
        hints_df[['effective','valid_number','sum_sim','target_words']]=hints_df.apply(lambda x: evaluate_hint(player,x.hint,x.number,board,model,0),axis=1)
        hints_df2=hints_df[hints_df.effective==1].sort_values(['valid_number','sum_sim'],ascending=False)
        if len(hints_df2)>0:
            hint=tuple(hints_df2.iloc[0][['valid_number','hint','target_words']])
            print(f'{player} hint {hint[1]} targeted: {hint[2]}')
